...
_PUG_TO_IMAGENET_MAPPING_CACHE = None
...
def get_classnames(dataset_name, data_root=None):
    if dataset_name.lower().startswith('pug'):
        if data_root is None:
            data_root = os.environ.get('D2O_DATA_ROOT', './data')

        pug_root = os.path.join(data_root, 'PUG_ImageNet')
        alias = {
            'cpitch': 'Camera_Pitch',
            'croll': 'Camera_Roll',
            'cyaw': 'Camera_Yaw',
        ...
        if key in ('pug', 'pug_imagenet'):
            variant = 'Worlds'
        elif key.startswith('pug_imagenet_'):
            variant = dataset_name[len('pug_imagenet_'):]
        elif key.startswith('pug_'):
            variant = dataset_name[len('pug_'):]
        else:
            variant = 'Worlds'

        variant = alias.get(variant.lower(), variant)
        variant_dir = os.path.join(pug_root, variant)
        if not os.path.isdir(variant_dir):
            variant_title = '_'.join([p[:1].upper() + p[1:] for p in re.split(r'[_\-]+', variant) if p])
            variant_dir = os.path.join(pug_root, variant_title)
        if not os.path.isdir(variant_dir):
            raise FileNotFoundError(f"Cannot find PUG_ImageNet variant directory: {variant_dir}")
        ...
        def _load_pug_to_imagenet_mapping():
            global _PUG_TO_IMAGENET_MAPPING_CACHE
            if _PUG_TO_IMAGENET_MAPPING_CACHE is not None:
                return _PUG_TO_IMAGENET_MAPPING_CACHE

            try:
                import pandas as pd
            except Exception as e:
                raise RuntimeError("Missing pandas dependency; cannot read the PUG-ImageNet mapping xlsx.") from e

            excel_path = os.path.join(os.path.dirname(__file__), 'PUG_ImageNet-Class_Ref_ImageNet_class.xlsx')
            if not os.path.exists(excel_path):
                raise FileNotFoundError(f"Cannot find mapping excel: {excel_path}")

            df = pd.read_excel(excel_path)
            pug_col = 'PUG \u7c7b\u522b\u540d (\u539f\u59cb)'
            img_col = 'ImageNet \u6807\u51c6\u7c7b\u522b\u540d'
            if pug_col not in df.columns or img_col not in df.columns:
                raise RuntimeError(f"Mapping excel columns mismatch. Got: {df.columns.tolist()}")

            alias = {
                'carton': 'cardboard box / carton',
                'gas mask': 'gas mask or respirator',
                'loupe': 'loupe magnifying glass',
                'street sign': 'traffic or street sign',
            }

            norm_to_imagenet = { _normalize_name(c): c for c in imagenet_classes }
            mapping = {}
            for _, row in df.iterrows():
                pug_name = str(row[pug_col]).strip()
                img_name_raw = str(row[img_col]).strip()

                img_name = re.split(r'\s*\(', img_name_raw, maxsplit=1)[0].strip()
                img_name = alias.get(img_name.lower(), img_name)

                norm = _normalize_name(img_name)
                if norm not in norm_to_imagenet:
                    raise RuntimeError(
                        f"ImageNet class name in mapping file cannot be aligned: "
                        f"'{img_name_raw}' -> '{img_name}'"
                    )

                mapping[pug_name] = norm_to_imagenet[norm]

            _PUG_TO_IMAGENET_MAPPING_CACHE = mapping
            return _PUG_TO_IMAGENET_MAPPING_CACHE

        pug_to_imagenet = _load_pug_to_imagenet_mapping()
        ...
        if len(folders) == 0:
            raise RuntimeError(f"No class folders found in: {variant_dir}")

        canonical_names = []
        for folder in folders:
            if folder not in pug_to_imagenet:
                raise RuntimeError(f"PUG_ImageNet class is missing from the mapping file: {folder}")
            canonical_names.append(pug_to_imagenet[folder])
        return canonical_names

    if 'imagenet' not in dataset_name:
        return eval("{}_classes".format(dataset_name.lower()))

    assert dataset_name in ['imagenet_a', 'imagenet_r', 'imagenet_sketch', 'imagenetv2', 'imagenet']
    if dataset_name == 'imagenet':
        return imagenet_classes

    def _read_wnid_to_name(file_path):
        wnid_to_name = {}
    ...
    dataset_to_classnames_relpath = {
        'imagenet_a': os.path.join('imagenet-adversarial', 'classnames.txt'),
        'imagenet_r': os.path.join('imagenet-rendition', 'classnames.txt'),
        'imagenet_sketch': os.path.join('imagenet-sketch', 'classnames.txt'),
        'imagenetv2': os.path.join('imagenetv2', 'classnames.txt'),
    }
    dataset_to_data_relpath = {
        'imagenet_a': os.path.join('imagenet-adversarial', 'imagenet-a'),
        'imagenet_r': os.path.join('imagenet-rendition', 'imagenet-r'),
        'imagenet_sketch': os.path.join('imagenet-sketch', 'ImageNet-Sketch'),
        'imagenetv2': os.path.join('imagenetv2', 'imagenetv2-matched-frequency-format-val'),
    }

    classnames_path = os.path.join(data_root, dataset_to_classnames_relpath[dataset_name])
    dataset_dir = os.path.join(data_root, dataset_to_data_relpath[dataset_name])

    if not os.path.exists(classnames_path):
        raise FileNotFoundError(f"Cannot find classnames.txt for {dataset_name}: {classnames_path}")
    if not os.path.isdir(dataset_dir):
        raise FileNotFoundError(f"Cannot find dataset directory for {dataset_name}: {dataset_dir}")

    wnid_to_name = _read_wnid_to_name(classnames_path)
    wnids = sorted([
    ...
    return [wnid_to_name.get(wnid, wnid) for wnid in wnids]
...
CUSTOM_TEMPLATES = {
    ...
    "pug_imagenet": ["a photo of a {}."],
    "imagenet_sketch": ["a sketch photo of a {}."],
    "imagenetv2": ["a photo of a {}."],
    "imagenet_a": ["a photo of a {}."],
    "imagenet_r": ["a rendition photo of a {}."],
    "imagenet_c": ["a photo of a {}."]   # Add the original ImageNet prompt template for ImageNet-C.
}
